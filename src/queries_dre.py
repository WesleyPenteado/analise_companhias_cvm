...
def get_analise_horizontal_dre(empresa, grupo):
    '''Retorna a tabela de análise horizontal da DRE de acordo com a empresa e grupo selecionados'''
    query = f"""
    WITH dados AS (
        SELECT
            CD_CONTA AS Conta,
...
def get_analise_vertical_dre(empresa, grupo):
    '''Retorna a tabela de análise vertical da DRE de acordo com a empresa e grupo selecionados'''
    query = f"""
    WITH dados AS (
        SELECT
            CD_CONTA AS Conta,
    ...
    SELECT
        ...
        Ano_2021,
        ROUND(
            (Ano_2021 * 100.0) / NULLIF((SELECT VL_CONTA FROM receita WHERE ano = 2021), 0),1
        )AS AV_2021,
        Ano_2022,
        ROUND(
            (Ano_2022 * 100.0) / NULLIF((SELECT VL_CONTA FROM receita WHERE ano = 2022), 0),1
        )AS AV_2022,
        Ano_2023,
        ROUND(
            (Ano_2023 * 100.0) / NULLIF((SELECT VL_CONTA FROM receita WHERE ano = 2023), 0),1
        )AS AV_2023,
        Ano_2024,
        ROUND(
            (Ano_2024 * 100.0) / NULLIF((SELECT VL_CONTA FROM receita WHERE ano = 2024), 0),1
        )AS AV_2024,
        Ano_2025,
        ROUND(
            (Ano_2025 * 100.0) / NULLIF((SELECT VL_CONTA FROM receita WHERE ano = 2025), 0),1
        )AS AV_2025
    FROM dados
    ORDER BY Conta;
    """
